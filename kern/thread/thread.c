...
//**************************************
int max_pid = 1;  // pid must be greater than 0
struct pnode *phead;  // store all created threads
//**************************************
...
void set_excode(int pid,int exitcode){
  ...
  while(dummy->next != NULL){
    if(dummy->next->pid == pid){
      dummy->next->excode = exitcode;
      //automatically it is set to be exitted
       dummy->next->exited = 1;
      break;
    }
    dummy = dummy->next;
...
void set_parentpid(pid_t parent,pid_t current){
  ...
  while(dummy->next != NULL){
    if(dummy->next->pid == current){
      dummy->next->parentpid = parent;
    }
    dummy = dummy->next;
  }
}
...
int check_parent_child_relationship(pid_t parent,pid_t current){
  ...
  while(dummy->next != NULL){
    if(dummy->next->pid == current){
      if(dummy->next->parentpid == parent){
...
int check_waited_by_pid(pid_t pid){
  ...
  while(dummy->next != NULL){
    if(dummy->next->pid == pid){
      if(dummy->next->waited == 1){
        return 1;  // is waited
      } else {
        return 0;  // is not waited
      }
    }  
    dummy = dummy->next;  
  }
  return 0;  // no such pid 'pid'
}
//**************************************
...
thread_bootstrap(void)
{
  ...
  struct thread *me;
	...
	/* Done */
  DEBUG(DB_THREADS, "Thread booted\n");
	return me;
}
	    ...
	    struct thread **ret)
{
  ...
  //**************************************
  /* Add child thread to child list */
  int i;
  for(i=0;i<MAX_CHILD;i++){
  ...
  if(curthread->child[i]==NULL){
    kprintf("Fail to add child thread\n");
  }
  newguy->parent = curthread;
  set_parentpid(curthread->pid,newguy->pid);
  //**************************************
...
{
  //**************************************
  // Dispose added things in thread structure
...
thread_wakeup_one(const void *addr)
{
    ...
    for (i=0; i<array_getnum(sleepers); i++) {
        ...
        if (t->t_sleepaddr == addr) {
            ...
            /*
             * As only one thread should be waken up,
             * the loop is broken once one thread
             * associated with the argument address is found
             */
            break;
        }
    }
