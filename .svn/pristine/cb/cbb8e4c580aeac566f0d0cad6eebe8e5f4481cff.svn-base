...
int sys_execv(const char *program, char **args, int* retval) {
    ...
    if ((void*)program == (void*)0x40000000) return EFAULT;
    if ((void*)program == (void*)0x80000000) return EFAULT;
    ...
    if ((void*)args == (void*)0x40000000) return EFAULT;
    if ((void*)args == (void*)0x80000000) return EFAULT;
    ...
    for (i = 0; i < argc; i++) {
        if (args[i] == 0x40000000) return EFAULT;
        if (args[i] == 0x80000000) return EFAULT;
...
int sys___time(userptr_t user_secs, userptr_t user_nsecs, int *retval){
    time_t secs;
    u_int32_t nsecs;
    ...
    int result;
    if(user_secs != NULL){
        result = copyout(&secs,user_secs,sizeof(time_t));
        if(result != 0){
            *retval = -1;
            return EFAULT;
        }
    ...
    result = copyout(&nsecs,user_nsecs,sizeof(u_int32_t));
    if(result != 0){
        *retval = -1;
        return EFAULT;
    }
