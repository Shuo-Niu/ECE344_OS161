...
//	}
	...
	// kprintf("cmd_progthread - progname %s, args[0]: %s, args[1]: %s, args[2]: %s\n", progname, args[0], args[1], args[2]);
...
#endif
    struct thread* cmd_thread;
	result = thread_fork(args[0] /* thread name */,
			args /* thread arg */, nargs /* thread arg */,
			cmd_progthread, &cmd_thread);
	if (result) {
		kprintf("thread_fork failed: %s\n", strerror(result));
		return result;
	}

	// create variables to use sys_waitpid
	int status,retval;
	int i = sys_waitpid(cmd_thread->pid,&status,0,&retval);
	return 0;
}
...
{
  if (nargs != 3) { // nargs -> number of arguments
    kprintf("Usage: df nr on/off\n");
    return EINVAL;
  }

  if (strcmp(args[2],"on") != 0 && strcmp(args[2],"off") != 0) { // args[2] -> should be "on" or "off"
    kprintf("Usage: df nr on/off\n");
    return EINVAL;
  }

  int flag = atoi(args[1]);
  switch (flag) { // atoi(args[1]) -> should between 1 to 12
  case 1:
...
static
int
cmd_opdbflags(int n, char **a)
{
	(void)n;
	(void)a;

	showmenu("OS/161 Debug flags", opdbflags);
...
menu(char *args)
{
	...
	while (1) {
    DEBUG(DB_DEVICE, "Displaying menu...\n");
